...
运行一个 Web 应用时，如果需要管理一堆文件的路径和权限的话，这个优势便不复存在。

解决方法是把必要的文件编译到应用二进制里去。在 Go 中可以用字节切片存放文件中
[String literals][string-literals] 形式的字符内容。
...
* 编译时更占内存
    * 如果你用的是小内存的设备开发，这会影响到你。但对我个人来说，还无需关心。

你需要在开发效率和运维管理时间上作取舍。如果你的应用受众是普罗大众（或者至少是那些有
个人网上应用的极客用户），就更值得权衡利弊了。

# 嵌入方式选择
第一个处理 Go 嵌入静态文件的库，或者说第一个真正知名的，大概是 [jteeuwen 的 Go-BinData][go-bindata]。
这是个命令行应用，你输入一个文件路径，它会生成包含静态文件的 `.go` 文件。

然而，作者 jteeuwen 似乎离开了星球，并在走时候把 GitHub 上这个仓库中所有东西删了。
幸运的是，他的代码是开源的，并在社区被广泛 fork。你在 GitHub 能找到好几个维护的很好的 fork。
...
本文目的是帮你弄明白这些库的差别，并帮你决定选择上面的库时你需要考虑的特性。

# 去芜存菁
有了这么多选项，当决定要用哪一个才最适合需求时就让人头大。你的程序和我的可能不一样，
但如果是 Web 应用的话，那会有很多重叠的地方。如果你需要做相同的选择时，下面的比较
将会很有用。
...
因为除了节省空间外，构建时内存占用量也会减少。此外，网页静态文件压缩率较高（比起
图片），这就是为什么大多数浏览器支持 gzip 压缩文件的原因。这就引入了我的下一个标准。

### 可选解压缩
如果你的可执行文件中存有 gzip 压缩的静态文件，并且你想把这些文件按 gzip 格式提供
给客户端，那为何不直接发送这些已经压缩的文件呢？理想的库应该支持一个选项，让你在
运行时去设定接受压缩后的文件，而不需要先解压。

### 从本地文件系统加载
在你开发 Web 应用时，任何在你修改代码和看到改变之间，增加了时间或困难的事，都应该
...
我选择它主要是因为我对原始的 go-bindata 库熟悉，而且这个 fork 看起来维护的很好。

这个库用起来很赞，但出乎意料的是，我的持续集成（CI）构建开始失败。像每一位碰到测试失败
的开发者一样，我想到的是**改动了什么**。我立刻去检查最新提交的改动，并试着弄明白为什么
这些改动导致模版处理失败。在我绞尽脑汁一阵后，没有找到原因，于是我重新跑了一次测试套件，
这次测试是基于最后一次成功的构建的，但也失败了。这说明改动在于环境，而不是我的代码。
不过这带来了更多了问题。

我是在 Docker 容器中跑持续集成测试的。测试环境应该是自我包好的、崭新的并且可复现的。
但这假设并不对。在我看 `Dockerfile` 时候，找到了问题所在：
...
就和我的预期不一样了。这可以归结为几个因素，比如 go get 总是获取默认的分支。最后，
可知是简单的原因：程序中生成代码的模块没有指定版本并让我的 git 仓库追踪。程序依赖的
外部代码如果改动了，就会在不知不觉时，导致我的构建或测试失败。

一种解决办法是存个预先编译的可用的 go-bindata 可执行文件到我的 git 项目中，但是：

1. 通常 git 仓库中不宜存二进制文件。
2. 如果依赖的库修复了漏洞，我每次都需要手动更新 go-bindata。
3. 如果换一个平台，将会使构建变得困难。

另外我找到了一个无需独立可执行文件的库，完全依赖代码版本，这可以用 vendor 来管理。
意思是一个支持 go-generate 的库，但特别地，不需要依赖外部程序运行的库。
...
#### GitHub 超过 200 星
这有点武断，不一定是衡量代码质量的方法。不过项目星数能说明库的活跃性，起码是个在多处被引用的库。
这反过来也说明很多人都在测试它，和（或）反馈问题。我选的库，仅仅超过了这个星数，注意一下。
...
我的经验是用这些库在写程序和部署时各有不同，要去看对应项目的 `README` 和文档。
如果你看到上表中不准确之处，请在评论中让我知道，我会更新。

# 我的选择
通过上面的比较表，就很清楚为什么我最终会用 [vfsgen][vfsgen]，并且我强烈推荐，
尤其在你需要可重现的构建时。稍逊一点的是 [fileb0x][fileb0x]，它需要独立的
